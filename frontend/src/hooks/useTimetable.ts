    ...
    try {
      ...
      if (rawEntries.length > 0) {
        const mapped: SlotEntry[] = rawEntries.map((e: any) => ({
          ...
          day: e.day,
          period: e.period,
          subjectCode: e.subject || e.raw_subject_text || e.subjectCode || 'CLASS',
          roomCode: e.room || e.raw_room_text || e.roomCode || 'TBD',
          facultyName: Array.isArray(e.faculty) ? e.faculty.join(', ') : (e.faculty || e.facultyName || 'Staff'),
          subjectType: e.entry_type || e.subjectType || 'L',
