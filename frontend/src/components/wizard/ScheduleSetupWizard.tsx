    ...
    <div className="space-y-6">
      ...
      {selectedSections.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
          ...
          {/* ── GENERATED TIMETABLE INLINE PREVIEW ──────────────────── */}
          {result && (
            <div className="border-t border-slate-100 dark:border-slate-800">
              {/* Success banner */}
              <div className="mx-6 mt-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
              ...
              <div className="px-6 pt-4">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Preview Section:</span>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedSections.map((sec) => (
                      <button
                        key={sec}
                        onClick={() => loadPreview(sec, (result as any).version_id)}
                        className={`px-3 py-1 rounded-full text-xs font-bold transition-all cursor-pointer ${
                          previewSection === sec
                            ? "bg-blue-600 text-white shadow-sm"
                            : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"
                  ...
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-700 dark:text-slate-300">
                        Generated Timetable — {previewSection}
                      </span>
                      <span className="text-[11px] text-slate-400 font-medium">
                        {previewEntries.length} scheduled slots
                      </span>
                    </div>
                    <div className="overflow-x-auto">
                      {previewEntries.length > 0 ? (
                        ...
                        <div className="py-10 text-center text-xs text-slate-400 font-medium">
                          No timetable entries found for {previewSection}. The solver may have saved to a different version — try opening the Full Grid.
                        </div>
                      )}
                    </div>
                  </div>
